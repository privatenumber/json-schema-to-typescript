            ...
            if (ast.type === 'INTERSECTION') {
                ...
                if (optimizedAST.params.length === 2) {
                    ...
                    if (first.type === 'INTERFACE' && second.type === 'UNION') {
                        const firstType = (0, generator_1.generateType)(first, options);
                        ...
                        for (const unionParam of second.params) {
                            if (unionParam.type === 'INTERFACE') {
                                const unionParamType = (0, generator_1.generateType)(unionParam, options);
                                if (firstType === unionParamType) {
                                    ...
                                    if (otherMembers.length > 0) {
                                        (0, utils_1.log)('cyan', 'optimizer', 'A & (A | ...) -> A | ...', optimizedAST);
                                        // Preserve standaloneName, keyName, comment from the intersection
                                        return Object.assign(Object.assign({}, second), { standaloneName: optimizedAST.standaloneName, keyName: optimizedAST.keyName, comment: optimizedAST.comment, deprecated: optimizedAST.deprecated });
                                    }
                                }
                            }
                        }
                    ...
                    if (second.type === 'INTERFACE' && first.type === 'UNION') {
                        const secondType = (0, generator_1.generateType)(second, options);
                            ...
                            if (unionParam.type === 'INTERFACE') {
                                ...
                                if (secondType === unionParamType) {
                                    ...
                                    if (otherMembers.length > 0) {
                                        (0, utils_1.log)('cyan', 'optimizer', '(A | ...) & A -> A | ...', optimizedAST);
                                        // Preserve standaloneName, keyName, comment from the intersection
                                        return Object.assign(Object.assign({}, first), { standaloneName: optimizedAST.standaloneName, keyName: optimizedAST.keyName, comment: optimizedAST.comment, deprecated: optimizedAST.deprecated });
                                    }
                                }
                            }
                        }
                ...
                if (optimizedAST.params.length > 2) {
                    const uniqueParams = (0, lodash_1.uniqBy)(optimizedAST.params, _ => (0, generator_1.generateType)(_, options));
                    if (uniqueParams.length < optimizedAST.params.length) {
                        (0, utils_1.log)('cyan', 'optimizer', 'Intersection with duplicates simplified', optimizedAST);
                        optimizedAST.params = uniqueParams;
                        // If we're left with just 1 param, return it directly
                        if (uniqueParams.length === 1) {
                            (0, utils_1.log)('cyan', 'optimizer', 'Single-member intersection unwrapped', optimizedAST);

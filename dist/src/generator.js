...
function declareNamedTypes(ast, options, rootASTName, processed = new Set(), outputtedNames = new Set()) {
    if (processed.has(ast)) {
        return '';
    }
    processed.add(ast);
    // Helper to generate standalone type only if name hasn't been used
    const generateStandaloneTypeOnce = (ast) => {
    ...
    switch (ast.type) {
        case 'ARRAY':
            return [
                ...
                (0, AST_1.hasStandaloneName)(ast) ? generateStandaloneTypeOnce(ast) : undefined,
            ]
                .filter(Boolean)
                .join('\n');
        case 'ENUM':
